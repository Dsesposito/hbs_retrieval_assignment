...
The implemented change is the semantic search. I chose the sentence embedding model 'Snowflake/snowflake-arctic-embed-m' because of its good performance, especially considering it's a model with 109 million parameters. See: https://huggingface.co/Snowflake/snowflake-arctic-embed-m.

This model can embed up to 512 tokens. When analyzing product descriptions and names, I observed that some products have over 4317 characters. A general rule of thumb is to consider a 3:1 ratio of characters to tokens, meaning the model can effectively embed up to 1536 characters without losing information.

To handle products with longer information, I use a text splitter from LangChain. The maximum length is set to 1536 characters, and a 50% overlap is added to prevent context loss during splitting.

The semantic search approach involves three steps:

1. Compute the embedding matrix.
2. For each query, perform a semantic search by encoding the query and then computing the cosine similarity. If a product has multiple chunks, keep the most similar one.
3. Sort the results by descending order of relevance and slice to the top_n=10.
4. Compute the MAP and Weighted MAP metrics.

As expected, semantic search performs better than lexical search due to its capacity to place related products close to each other.
